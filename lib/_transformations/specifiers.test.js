...
import { transpileSpecifier } from "./specifiers.js";
...
const scopedPackage = fc.tuple(name.map((s) => s ? `@${s}/` : ""), name.filter(Boolean)).map(join);
const service = fc.constantFrom("npm:", "https://esm.sh/", "https://cdn.skypack.dev/");
test(function localSpecifiers() {
    fc.assert(fc.property(relativePath, extension, (path, ext) => transpileSpecifier(`${path}.deno.${ext}`) === `${path}.node.js`));
});
test(function remoteSpecifiers() {
    fc.assert(fc.property(service, scopedPackage, version, path, query, (...segments) => {
