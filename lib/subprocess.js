...
const [lsStdout, lsStderr] = await Promise.all([lsProcess.output(), lsProcess.stderrOutput()]);
...
// Read the output and error asynchronously
const [shStdout, shStderr] = await Promise.all([shProcess.output(), shProcess.stderrOutput()]);
// Decode and log the output
if (shStdout) {
